    ...
    <main style={page}>
      <div style={wrap}>
        ...
        <nav style={{ marginTop: 56, paddingTop: 24, borderTop: "1px solid rgba(23,54,41,.18)", display: "flex", gap: 18, flexWrap: "wrap", fontSize: 13 }}>
          <Link href="/chalet" style={link}>Chalet</Link><Link href="/superior" style={link}>Superior Chalet</Link><Link href="/faq" style={link}>FAQ</Link><Link href="/terms" style={link}>Booking terms</Link><Link href="/privacy" style={link}>Privacy</Link><Link href="/cookies" style={link}>Cookies</Link><Link href="/legal" style={link}>Legal notice</Link>
        </nav>
      </div>
    </main>
  );
